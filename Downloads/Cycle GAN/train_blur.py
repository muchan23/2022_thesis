    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        # Train Discriminators O and S
            with torch.cuda.amp.autocast():
                ...
                D_O_real_loss = mse(D_O_real, torch.full_like(D_O_real,0.9)) #label smooth
                D_O_fake_loss = mse(D_O_fake, torch.full_like(D_O_fake,0.1)) #label smooth
                ...
                D_S_real_loss = mse(D_S_real, torch.full_like(D_S_real,0.9)) #label smooth
                D_S_fake_loss = mse(D_S_fake, torch.full_like(D_S_fake,0.1)) #label smooth
            ...
            with torch.cuda.amp.autocast():
                ...
                loss_G_O = mse(D_O_fake, torch.full_like(D_O_fake,0.9)) #label smooth
                loss_G_S = mse(D_S_fake, torch.full_like(D_S_fake,0.9)) #label smooth
            ...
            if idx % 200 == 0:
                save_image(fake_optical*0.5+0.5, f"/home/yuki_murakami/ドキュメント/Cycle GAN/data/lr=5e-5,C=10,I=5,SNL,blur/fakeoptical/optical_{idx}.jpeg") #check
                save_image(fake_sar*0.5+0.5, f"/home/yuki_murakami/ドキュメント/Cycle GAN/data/lr=5e-5,C=10,I=5,SNL,blur/fakesar/sar_{idx}.jpeg") #chec
            ...
            #25は手動で変える必要がある    
            if idx % 500 == 0:
                writer = SummaryWriter(log_dir="./TensorBoard/Graph/lr=5e-5,C=10,I=5,SNL,blur") #check
                ...
                writer1 = SummaryWriter("./TensorBoard/Image/lr=5e-5,C=10,I=5,SNL,blur")
        ...
        if config.SAVE_MODEL:
            save_checkpoint(gen_O, opt_gen, filename=f"/home/yuki_murakami/ドキュメント/Cycle GAN/data/lr=5e-5,C=10,I=5,SNL,blur/checkpoint/epoch{epoch+1}/geno.pth.tar")
            save_checkpoint(gen_S, opt_gen, filename=f"/home/yuki_murakami/ドキュメント/Cycle GAN/data/lr=5e-5,C=10,I=5,SNL,blur/checkpoint/epoch{epoch+1}/gens.pth.tar")
            save_checkpoint(disc_O, opt_disc, filename=f"/home/yuki_murakami/ドキュメント/Cycle GAN/data/lr=5e-5,C=10,I=5,SNL,blur/checkpoint/epoch{epoch+1}/critico.pth.tar")
            save_checkpoint(disc_S, opt_disc, filename=f"/home/yuki_murakami/ドキュメント/Cycle GAN/data/lr=5e-5,C=10,I=5,SNL,blur/checkpoint/epoch{epoch+1}/critics.pth.tar")

        print(epoch)

    np.save("/home/yuki_murakami/ドキュメント/Cycle GAN/data/lr=5e-5,C=10,I=5,SNL,blur/numpy/G_loss_iter",G_loss_iter)
    np.save("/home/yuki_murakami/ドキュメント/Cycle GAN/data/lr=5e-5,C=10,I=5,SNL,blur/numpy/D_loss_iter",D_loss_iter)
    np.save("/home/yuki_murakami/ドキュメント/Cycle GAN/data/lr=5e-5,C=10,I=5,SNL,blur/numpy/G_loss_epoch",G_loss_epoch)
    np.save("/home/yuki_murakami/ドキュメント/Cycle GAN/data/lr=5e-5,C=10,I=5,SNL,blur/numpy/D_loss_epoch",D_loss_epoch)
